  ...
  >
    <div class="contentBox fWhite">
      ...
      <div class="flexRowInfo">
        ...
        <div class="flexRow">
          <div
            v-for="item in monyTypes"
            :key="item.id"
            class="grid"
            :class="{ activeGrid: item.id === selectMony }"
            @click="changeSelectMoney(item.id, item.amount)"
          >
            {{ item.amount }}HK$
          </div>
        </div>
      ...
      <div class="flexRowInfo">
        ...
        <div class="flexRow pay">
          <div
            ...
            :class="{ active: item.id === selectWayId }"
            :key="item.id"
            @click="changePayWay(item.id)"
          >
            <span
              class="wayName"
              ...
              ><i
                class="iconfont"
                :class="[item.icon, { active: item.id === selectWayId }]"
              ></i
              >{{ item.name }}</span
            >
          </div>
      ...
      API.getChargeList({
        ...
        if (result.code !== 200) {
          this.common.notification(
            this,
            "Failed to query user recharge records. Procedure",
            3
          );
          return;
        }
    ...
    charge() {
      if (this.defineAmount <= 0) {
        if (this.selectMony == "0") {
          this.common.notification(
            this,
            "Please enter the correct recharge amount",
            3
          );
        } else {
          this.common.notification(
            this,
            "Please select the amount to be recharged",
            3
          );
        }
        return;
    ...
    startAliPay() {
      ...
      if (this.payType == 2) {
        API.startAliPay({
          ...
          console.log("Form the form: " + JSON.stringify(result));
          ...
          setTimeout(function () {
            $("#alipay-wap").remove(); // 拉起Alipay后移除dom
          }, 1000);
          this.loading = false;
        });
      }
